...
| **0.0.13** | [0.0.13-eng-5-stages-spec.md](./0.0.13-eng-5-stages-spec.md) | **5 阶段 agent 输入输出规范 (DEC-2026-0003)** | 5 阶段 agent 物理位置业务项目内, PMO 给规范不实施 |
...
## DEC-2026-0003 调整 (2026-06-19)

Sponsor 推动 5 阶段 agent 修正, 增加 2 项元规则:
- 0.0.13 5 阶段 agent 输入输出规范 (5 阶段 agent 物理位置业务项目内, PMO 给规范不实施)
- 0.0.14 业务需求运营 agent 自定原则 (业务 agent 业务项目完全自定, PMO 不预设不干预)

**调整原因**: 之前 0.0.11/0.0.12 写"5 阶段 agent PMO 预设"含义模糊, 业务项目接入缺模板, 业务项目内 2 类 agent 边界未明文。Sponsor 关键点出: 5 阶段 agent 是业务项目内实施的 agent, PMO 只是标准约束监督的职责。
